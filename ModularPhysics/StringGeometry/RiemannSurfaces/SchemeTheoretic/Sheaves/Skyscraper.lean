...
import ModularPhysics.StringGeometry.RiemannSurfaces.SchemeTheoretic.Helpers.SkyscraperCohomology
...
instance skyscraperModule_isCoherent (p : C.PointType) :
  ...
  locallyFinitelyGenerated := fun i => by
    ...
    -- which is κ(p) if p ∈ U, else PUnit (the zero module)
    ...
    change Module.Finite ↑(C.toScheme.presheaf.obj U)
      ↑(SkyscraperConstruction.skyscraperObj (X := C.toScheme) p U)
    by_cases hp : (p : C.toScheme.carrier) ∈ U.unop
    · -- p ∈ U: sections are κ(p), finitely generated by {fromKappa(1)}
      -- fromKappa(1) generates the module: every x = evalAtPoint(r) • fromKappa(1) for some r
      ...
      intro x
      -- Get the κ(p) value of x via toKappa
      let v := SkyscraperConstruction.toKappa (X := C.toScheme) p U hp x
      -- By surjectivity of canonical map ℂ → κ(p), find c with canonical(c) = v
      obtain ⟨c, hc⟩ := (C.residueFieldIsComplex p).2 v
      -- Factor: ℂ → Γ(X,⊤) → O_X(U) → κ(p) to get the ring element r
      let globalSec : Γ(C.toScheme, ⊤) :=
        C.structureMorphism.appTop.hom ((Scheme.ΓSpecIso (CommRingCat.of ℂ)).inv.hom c)
      let r : ↑(C.toScheme.presheaf.obj U) :=
        (C.toScheme.presheaf.map (homOfLE le_top).op).hom globalSec
      refine ⟨r, ?_⟩
      -- Show: r • fromKappa(1) = x
      -- Apply toKappa (injective) to reduce to κ(p) level
      apply SkyscraperConstruction.toKappa_injective (X := C.toScheme) p U hp
      -- Simplify toSpanSingleton application to r • fromKappa(1)
      simp only [LinearMap.toSpanSingleton_apply]
      -- toKappa(r • fromKappa(1)) = evalAtPoint(r) * toKappa(fromKappa(1))
      ...
      -- r = res_{U≤⊤}(globalSec), use evalAtPoint_comp_restriction
      ...
      rw [SkyscraperConstruction.evalAtPoint_comp_restriction
        p U.unop ⊤ hp (Set.mem_univ _) le_top globalSec]
...
/-- h⁰(k_p) = 1.
    ...
    **Proof:**
    H⁰(C, k_p) = CechCocycles(k_p, 𝒰, 0) is 1-dimensional over ℂ.
    ...
    2. `skyscraper_cocycle_scalar_multiple`: every cocycle is a ℂ-scalar multiple
       of the constant 1 cocycle

    **Key infrastructure from SkyscraperCohomology.lean:**
    - `constantCocycle` : nonzero element of H⁰(k_p)
...
theorem h0_skyscraper (C : ProperCurve) (p : C.toAlgebraicCurve.PointType) :
  ...
  unfold h_i
  -- SheafCohomology C 0 (skyscraperModule C p) = CechCocycles (skyPresheaf C p) 𝒰 0
  -- where 𝒰 = standardAffineCover C (all definitional)
  let 𝒰 := standardAffineCover C.toAlgebraicCurve
  -- Provide the ℂ-module instance on CechCocycles explicitly
  -- (CechCohomology0.module is registered on CechCohomology0, not CechCocycles directly)
  letI : Module ℂ (CechCocycles (SkyscraperH0.skyPresheaf C.toAlgebraicCurve p) 𝒰 0) :=
    CechCohomology0.module C.toAlgebraicCurve _ 𝒰
  -- finrank = 1 iff every element is a scalar multiple of a nonzero element
  apply (finrank_eq_one_iff_of_nonzero'
    (SkyscraperH0.constantCocycle C.toAlgebraicCurve p 𝒰 1)
    (SkyscraperH0.constantCocycle_one_ne_zero C.toAlgebraicCurve p 𝒰)).mpr
  intro w
  exact SkyscraperH0.skyscraper_cocycle_scalar_multiple C.toAlgebraicCurve p 𝒰 w
