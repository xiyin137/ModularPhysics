...
where E → M_red is a rank q smooth vector bundle over the body.

### Proof Outline (Batchelor 1979)

**Step 1: The nilpotent ideal**
Let J ⊂ O_M be the ideal of nilpotent elements (generated by odd elements).
The quotient O_M/J = O_{M_red} is the structure sheaf of the body.

**Step 2: The filtration**
J has a natural filtration by powers: J ⊃ J² ⊃ J³ ⊃ ... ⊃ J^{q+1} = 0.
Each quotient J^k/J^{k+1} is a locally free O_{M_red}-module.

**Step 3: The key short exact sequence**
    0 → J² → O_M → O_M/J² → 0

The quotient O_M/J² ≅ O_{M_red} ⊕ E* where E* = J/J² is a rank q vector bundle
...
structure NilpotentIdeal {dim : SuperDimension} (_ : Supermanifold dim) where
  /-- J ⊂ O_M is the ideal of odd elements -/
  ideal : True
  /-- J is locally generated by θ¹, ..., θ^q in any chart -/
  localGenerators : True
  /-- J^{q+1} = 0 (nilpotency) -/
  nilpotent : dim.odd + 1 > 0 → True
  /-- O_M/J = O_{M_red} -/
  quotientIsBody : True
...
/-- The odd cotangent bundle E* = J/J² is a rank q vector bundle over M_red.
...
/-- The splitting data for a split supermanifold.
    ...
    This packages the vector bundle E and the isomorphism O_M ≅ ∧•E*. -/
structure SplittingData {dim : SuperDimension} (M : Supermanifold dim) where
  /-- The vector bundle E → M_red -/
  bundle : OddCotangentBundle M
  /-- The isomorphism O_M ≅ ∧•E* of sheaves of superalgebras -/
  iso : True
...
theorem splitting_nonuniqueness {dim : SuperDimension} (M : Supermanifold dim)
    (S₁ S₂ : SplittingData M) :
    True := by  -- S₁ and S₂ differ by an element of a cohomology group
  trivial
