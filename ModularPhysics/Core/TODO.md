...
Verified via `grep -r "^axiom" ModularPhysics/Core/`: zero axioms across the entire Core directory, including all non-QFT modules (ClassicalMechanics, Quantum, SpaceTime, Symmetries, StatisticalMechanics, Thermodynamics, Condensed, Information).
...
The `t_dual` definition in `CFT/TwoDimensional/Examples.lean` has a sorry for the weight proof (T-duality swaps momentum/winding). This is acceptable for Core but could be proved with some algebraic manipulation.
